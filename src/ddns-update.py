...
def init_config_dir():
    ...
    try:
        ...
        if not os.path.exists(CONFIG_FILE):
            if os.path.exists(APP_CONFIG_EXAMPLE):
                shutil.copy2(APP_CONFIG_EXAMPLE, CONFIG_FILE)
                # 注意：此时日志系统可能还未初始化，使用print并立即刷新
                print(f"配置文件不存在，已从示例文件复制到: {CONFIG_FILE}", flush=True)
            else:
                error_msg = f"配置文件不存在且示例文件不存在: {APP_CONFIG_EXAMPLE}"
                raise FileNotFoundError(error_msg)
    except FileNotFoundError:
        # 重新抛出FileNotFoundError，让程序退出
        raise
    except Exception as e:
        error_msg = f"初始化配置目录时出错: {e}"
...
class DDNSClient:
    ...
    def run_once(self) -> bool:
        ...
        # 验证获取到的IP地址
        if not is_valid_ipv4(self.current_ip):
            logger.error(f"获取到的IP地址格式无效: {self.current_ip}")
            write_health_check()
            return False
